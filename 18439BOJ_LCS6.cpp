...
struct bset
{
    ...
    void print(){
        for (int i=0; i<length; ++i) cout << get(i);
        cout << '\n';
    }
};
...
void lcs(int l1, int l2, int r1, int r2){
    ...
    if (l1 == l2) {
        for (int i = r1; i<= r2; ++i){
            if (a[l1] == b[i]) result.push_back(b[i]); break;
        }
        return;
    }
    int mid = l1 + l2 >> 1;
    ...
    bset prv2 = lcs_pre(mid+1,l2,r1,r2);
    for (int i=1; i<=m; ++i) lcs1[i] = lcs1[i-1] + prv1.get(i);
    for (int i=m; i>=1; --i) lcs2[i] = lcs2[i+1] + prv2.get(i);
...
int main(){
    cin >> a >> b;
    bset prev = lcs_pre(0,a.size()-1, 0,b.size()-1);
    cout << prev.len() << '\n';
}