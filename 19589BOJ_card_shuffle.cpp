...
struct SplayTree
{
    ...
    void inord(Node *x, vector<int> &ord){
        prop(x);
        if (x->l) inord(x->l, ord);
        ord.push_back(x->val);
        if (x->r) inord(x->r, ord);
    }

    void change(Node *x, vector<int> &ord, int &pv){
        if (x->l) change(x->l, ord, pv);
        x->val = ord[pv++];
        if (x->r) change(x->r, ord, pv);
    }
