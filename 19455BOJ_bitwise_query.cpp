...
struct Node
{
    int mn, on, off, lazy_and, lazy_or;
    Node(){
        mn = INF; on = off = lazy_or = 0;
        lazy_and = INF;
    }
};
...
int query(int l, int r, int s=1, int e=n, int x=1){
    ...
    int mid = s + e >> 1;
    return min(query(l,r,s,mid,x<<1), query(l,r,mid+1,e,x<<1|1));
}
