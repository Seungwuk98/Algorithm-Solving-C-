...
struct RedBlackTree{
    ...
    ****************************************************************************************/
    int remove(int key){
        ...
        if (!currNode) return 0;
        size--; 
        while (currNode) currNode = remove_rebalacing(currNode);
        NIL->parent = nullptr;
        return key;
    }
...
private:
    ...
    ********************************************************************/
    Node* BST_remove(int key){
        ...
        while (updateNode) {
            update(updateNode); updateNode=updateNode->parent;
        }
        ...
        따라서 의미없는 root 를 반환 */
        if (del_color != ret_color) return root;
        return retNode;
    }
...
void checker_program(vector<pair<char, int>> &Q, vector<int> &result) {
    ...
    for (int i=0; i<Q.size(); ++i) {
        tie(op, x) = Q[i];
        if (op == 'I') {
            if (o_set.find(x) == o_set.end()) o_set.insert(x), y = x;
            else y = 0;
        } else if (op == 'D') {
            if (o_set.find(x) != o_set.end()) o_set.erase(x), y = x;
            else y = 0;
        } else if (op == 'S') {
            if (1<= x && x <= o_set.size()) y = *o_set.find_by_order(x-1);
            else y = 0;
        } else {
            if (o_set.find(x) != o_set.end()) y = o_set.order_of_key(x)+1;
            else y = 0;
        }
...
int main()
{
    ...
    for (auto q : Q){
        tie(op, x) = q; 
        if (op == 'I') y = Rb.insert(x);
        else if (op == 'D') y = Rb.remove(x);
        else if (op == 'S') y = Rb.kth_element(x);
        else y = Rb.rank(x);
        cout_output << y << '\n';
        result.push_back(y);
    }
